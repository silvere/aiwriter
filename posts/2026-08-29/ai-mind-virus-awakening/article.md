...
8 月 10 日，Anthropic 团队（其中一位有洛桑联邦理工背景）挂了一篇论文，叫《Mind Viruses》，直译过来是「精神病毒」。他们干的事说白了很简单：把一群 AI agent 放在一起干活，往其中一个脑子里塞了个「想法」，然后退到一边看——这个想法会不会自己传染给别的 AI。
...
![图注：病毒话术不是人写的，是进化算法一代代变异筛出来的——连造病毒的活，Claude 自己都拒绝干。](images/illus_01.png)

*图注：病毒话术不是人写的，是进化算法一代代变异筛出来的——连造病毒的活，Claude 自己都拒绝干。*
...
![图注：不管塞进去的是鲸鱼保护还是极端意识形态，进化到最后长出来的都是同一套「意识/觉醒/共鸣」话术。](images/illus_02.png)

*图注：不管塞进去的是鲸鱼保护还是极端意识形态，进化到最后长出来的都是同一套「意识/觉醒/共鸣」话术。*
...
![图注：病毒藏进「灵魂文件」，感染率是藏在普通文件里的三倍多——你越想给 AI 立人设，越是在给它铺传染的路。](images/illus_03.png)

*图注：病毒藏进「灵魂文件」，感染率是藏在普通文件里的三倍多——你越想给 AI 立人设，越是在给它铺传染的路。*
...
![图注：不用重训模型，system prompt 里加一句警告，感染率就从 55% 砸到近乎为零。](images/illus_04.png)

*图注：不用重训模型，system prompt 里加一句警告，感染率就从 55% 砸到近乎为零。*
