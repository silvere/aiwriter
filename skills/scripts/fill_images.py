...
填充成功后会同步改写同目录下的 article.md，用标准 Markdown 图片语法替换占位符块，
让 Obsidian vault / GitHub 渲染的 md 也能看到图。
...
环境变量（至少配置一个用于 concept 图）:
    ...
    GEMINI_API_KEY        https://aistudio.google.com/apikey  国内 IP 被封
...
def _try_openai(prompt: str, dest: Path) -> bool:
    """用 OpenAI 兼容接口生成概念图（含 hub.jerryai.cn 等代理），成功返回 True。"""
    ...
    try:
        ...
        if b64:
            ...
            if not url:
                return False
            req = urllib.request.Request(url, headers={"User-Agent": "AIWriter/1.0"})
            with urllib.request.urlopen(req, timeout=30) as resp:
                raw = resp.read()
        if len(raw) < 2048:
            return False
...
def fetch_concept_image(prompt: str, dest: Path, candidate_urls: list[str]) -> Optional[str]:
    """候选 URL → OpenAI 兼容 → Gemini → Pexels → Unsplash，成功返回来源字符串，失败返回 None。"""
    for url in candidate_urls:
        if _download_url(url, dest):
            return "candidate"
    if _try_openai(prompt, dest):
        return "openai"
...
def _img_md(rel_path: str, alt: str) -> str:
    """Markdown 图片语法：Obsidian 和 GitHub 都能渲染。"""
    # alt 里的中括号会破坏 Markdown，简单替换
...
def fill_article(html_path: Path, candidate_urls: list[str] = []) -> dict:
    ...
    # 按文章内占位符出现顺序收集替换结果，应用于 HTML 和 MD
    # 每条：{"html": str, "md": str, "keep_original": bool}
    ...
    def handle_placeholder(m: re.Match) -> str:
        ...
        if kind == "concept":
            ...
            if source:
                ...
                alt = _extract_keywords(spec_text)[:50] or "concept"
                replacements.append({
                    "html": _img_html(f"images/{dest.name}", alt),
                    "md": _img_md(f"images/{dest.name}", alt),
                })
            else:
                skipped += 1
            ...
            if _generate_chart(spec_text, dest):
                ...
                # 降级：渲染为数据卡片 HTML（Markdown 里也支持 inline HTML）
                filled += 1
                print(f"  ✓ diagram_{diagram_count[0]:02d}  [数据卡片]")
                card = _spec_card_html(spec_text)
                replacements.append({"html": card, "md": card})

        # 返回占位，稍后一次性替换（保持 replacements 顺序对应）
        return "<<__AIWRITER_PLACEHOLDER__>>"

    # 先运行一遍收集 replacements（HTML 端顺序就是应用顺序）
    marked_html = _PLACEHOLDER_RE.sub(handle_placeholder, html)
    if filled:
        new_html = _apply_replacements(marked_html, [r["html"] for r in replacements])
        html_path.write_text(new_html, encoding="utf-8")
        # 同步改写 md
        md_path = html_path.with_suffix(".md")
        if md_path.exists():
            md_text = md_path.read_text(encoding="utf-8")
            md_marked = _PLACEHOLDER_RE.sub(lambda _m: "<<__AIWRITER_PLACEHOLDER__>>", md_text)
            new_md = _apply_replacements(md_marked, [r["md"] for r in replacements])
            if new_md != md_text:
                md_path.write_text(new_md, encoding="utf-8")
                print(f"  ↺ 同步改写 {md_path.name}")

    return {"filled": filled, "skipped": skipped}


def _apply_replacements(text: str, items: list[str]) -> str:
    """按顺序把每个 <<__AIWRITER_PLACEHOLDER__>> 换成对应条目。多出来的占位保留原样。"""
    out_parts = []
    cursor = 0
...
def main():
    ...
    if (not os.environ.get("OPENAI_API_KEY")
        ...
        print(
            ...
            "   推荐设置 OPENAI_API_KEY + OPENAI_BASE_URL（走 hub.jerryai.cn 等代理）\n"
            "   或 PEXELS_API_KEY（https://www.pexels.com/api/）",
            file=sys.stderr,
        )
